...
try {
    ...
    $mail->Username = 'xxxxxxxxxxxx@163.com';          // SMTP 用户名  即邮箱的用户名
    $mail->Password = 'xxxxxxxxxxxxx';             // SMTP 密码  部分邮箱是授权码(例如163邮箱)
    $mail->SMTPSecure = 'ssl';                        // 允许 TLS 或者ssl协议
    $mail->Port = 465;                                // 服务器端口 25 或者465 具体要看邮箱服务器支持

    $mail->setFrom('xxxxxxxxxxxxxx@163.com', '西安工商学院技能证书管理');  //发件人
    $mail->addAddress($obj->email, 'qq');  // 收件人
    //$mail->addAddress('ellen@example.com');  // 可添加多个收件人
    $mail->addReplyTo('xxxxxxxxxxxx@163.com', 'info'); //回复的时候回复给哪个邮箱 建议和发件人一致
    
    $mail->isHTML(true);                                  // 是否以HTML文档格式发送  发送后客户端可直接显示对应HTML内容
    $mail->Subject = '本邮件由西安工商学院技能证书管理系统自动发送';
    $mail->Body    = '关于您在系统提交的证书：<br>'.$conclusion .'<br>发送日期：'. date('Y-m-d H:i:s');
